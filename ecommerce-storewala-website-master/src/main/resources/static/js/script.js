    ...
    if (cart == null || cart.length == 0) {
        ...
        cart.map((item) => {
            ...
            table += `
                <tr>
                <td>${item.productName}
                 ${productsNames += item.productName}
        ...
        table += `<tr><td colspan='5' class='text-center font-weight-bold'>Total Price: Rs.${formatCurrency.format(totalPrice)}/-
            <input type='hidden' id='cartTotalAmount' name='total_price' value='${totalPrice}'>
            <input type='hidden' id='cartProductName' name='product_name' value='${productsNames}'>
            </td></tr></table></div>`

        $(".cart-body").html(table);
        $(".checkout-btn").attr('disabled', false);
        document.getElementById('cartTotalAmount').value = totalPrice;
    }
}
...
//	let cartString = localStorage.getItem("cart");
//	let cart = JSON.parse(cartString);
////	$(".cart-body").html(`<input type="hidden" id="status" value=`${}` />`)
//	if (cart == null || cart.length == 0) {
//		console.log("Cart is empty!!");
//		$(".cart-items").html("0");
//		$(".cart-body").html(`<h1  class="text-center">Sorry, Cart doesn't have any items </h1>`);
